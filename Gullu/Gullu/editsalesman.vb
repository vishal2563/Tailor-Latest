...
Public Class editsalesman
    ...
    Private Sub editsalesman_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        RefreshAll()
    End Sub
    ...
    Private Sub ListBox2_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ListBox2.SelectedIndexChanged
        ...
        openconn()
        cmd = New SqlCommand("select * from salesman where smid=" & ListBox1.Text, cnn)
        drd = cmd.ExecuteReader
        If drd.Read Then
            TextBox1.Text = drd("smid")
            TextBox2.Text = drd("smname")
            TextBox4.Text = drd("mobile")
        End If
        drd.Close()

        cmd = New SqlCommand("select * from delivery where smid=" & ListBox1.Text, cnn)
        drd = cmd.ExecuteReader
    ...
    Private Sub btnsave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnsave.Click
        ...
        If ListBox2.Text <> "" And Trim(TextBox2.Text) <> "" And Len(TextBox4.Text) = 10 Then
            openconn()
            cmd = New SqlCommand("update salesman set smid=@smid,smname=@smname,mobile=@mobile where smid=" & ListBox1.Text, cnn)
            ...
            cmd = New SqlCommand("update delivery set smname='" & TextBox2.Text & "' where smid=" & ListBox1.Text, cnn)
            cmd.ExecuteNonQuery()
            RefreshAll()
        Else
    ...
    Private Sub btndel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btndel.Click
        If ListBox2.Text <> "" Then
            openconn()
            cmd = New SqlCommand("delete from salesman where smid=" & ListBox1.Text, cnn)
            cmd.ExecuteNonQuery()
            closeconn()
            RefreshAll()
        End If
    ...
    Public Sub RefreshAll()
        ...
        openconn()
        cmd = New SqlCommand("select * from salesman order by smid  ", cnn)
        drd = cmd.ExecuteReader
        While drd.Read
